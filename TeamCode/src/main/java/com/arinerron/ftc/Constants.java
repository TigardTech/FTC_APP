package com.arinerron.ftc;

public class Constants {
    /* shhh very secret key thing you didn't just read */
    public static final String LICENSE_KEY = ""; // shhh you didn't just read this
    public static final String ROBOT_NAME = "Operation Stripes";

    /* define motor names */
    public static final String ROTATING_MOTOR = "leftrm";
    public static final String RIGHT_MOTOR = "rightm";
    public static final String LEFT_MOTOR = "leftm";

    /* define sensor names. empty string disables sensor. */
    public static final String COLOR_SENSOR = "";
    public static final String GYRO_SENSOR = "";

    /* thresholds */
    public static final double DRIVING_THRESHOLD = 0.1;
    public static final double TRIGGER_THRESHOLD = 0.2;
}
